...
void MAXreg_wr(BYTE reg, BYTE val) {
    ...
    int return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 2, wdata, 0, 0, 0);
    if(return_code<0)
        alt_printf("1-byte register write error.\n");
    return;
}
//multiple-byte write
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int return_code = alt_avalon_spi_command(SPI_0_BASE, 0, nbytes+1, wdata, 0, 0, 0);
	if(return_code<0)
		alt_printf("n-byte register write error.\n");
//	return &data[nbytes];
	return (data + nbytes);
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	alt_u8 rdata;
	int return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, 1, &rdata, 0);
	if(return_code<0)
		alt_printf("1-byte register read error.\n");
	return rdata;
}
//multiple-byte read
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	if(return_code<0)
		alt_printf("n-byte register read error.\n");
	return (data + nbytes);
}
